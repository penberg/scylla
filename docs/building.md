...
Alternatively, you can also pass the `mode` directly to the `ninja-build` command:

```console
git submodule update --init --force --recursive
./tools/toolchain/dbuild ./configure.py
./tools/toolchain/dbuild ninja-build <mode>
```
...
As the first step, you need to run the `configure.py` script, which generates a `build.ninja` build file (similar to a `Makefile`) for the [Ninja] build tool that we use for building.
...
```console
./tools/toolchain/dbuild ninja-build build
```
...
```console
./tools/toolchain/dbuild ninja-build <mode>-build
```
...
```console
./tools/toolchain/dbuild ninja-build test
```

Alternatively, to run unit tests for a specific build mode, run:

```console
./tools/toolchain/dbuild ninja-build <dev>-test
```
