...
with open(buildfile_tmp, 'w') as f:
    ...
    for mode in build_modes:
        ...
        f.write('  pool = submodule_pool\n')
        f.write('  mode = {mode}\n'.format(**locals()))
        f.write('build build/dist/{mode}/redhat: rpmbuild build/{mode}/scylla-package.tar.gz\n'.format(**locals()))
        f.write('  pool = submodule_pool\n')
        f.write('  mode = {mode}\n'.format(**locals()))
        f.write('build build/dist/{mode}/debian: debbuild build/{mode}/scylla-package.tar.gz\n'.format(**locals()))
        f.write('  pool = submodule_pool\n')
        f.write('  mode = {mode}\n'.format(**locals()))
        f.write('build dist-server-{mode}: phony build/dist/{mode}/redhat build/dist/{mode}/debian\n'.format(**locals()))
    ...
    f.write(
            'build dist-server-deb: phony {}\n'.format(' '.join(['build/dist/{mode}/debian'.format(mode=mode) for mode in build_modes]))
    )
    f.write(
            'build dist-server-rpm: phony {}\n'.format(' '.join(['build/dist/{mode}/redhat'.format(mode=mode) for mode in build_modes]))
    )
    f.write('build dist-server: phony dist-server-rpm dist-server-deb\n')
    ...
    f.write(textwrap.dedent('''\
        ...
        ''').format(**globals()))
    f.write('build dist-deb: phony dist-server-deb dist-python-deb dist-jmx-deb dist-tools-deb\n')
